...
export function renderCallSheet(store: Store, root: HTMLElement, dayOverride?: import("./model").ShootDay) {
  ...
  let html = `<p class="page-label">${t("通告單 · A5 橫")}</p><div class="page callsheet">`;
  html += `
    <div class="cs-head">
      <span class="cs-title cut-edit" contenteditable draggable="false" data-meta="title" data-ph="${t("片名")}">${esc(p.meta.title)}</span>
      <span class="cs-tag">${t("拍攝通告單")}</span>
  ...
  html += `<div class="cs-groups"><div class="cs-gh">${t("大組通告時間")}</div><div class="cs-glist">`;
  day.callGroups.forEach((g, i) => {
    html += `<div class="cs-grow" data-cgrow="${i}">
      <span class="cs-grip" data-cgrip="${i}" title="${t("拖曳排序")}">⠿</span>
      <span class="cs-gl cut-edit" contenteditable draggable="false" data-cg="${i}" data-cgf="label" data-ph="${t("組別／演員")}">${esc(g.label)}</span>
      <span class="cs-gs"><span class="cs-dot">・</span><span class="cut-edit" contenteditable draggable="false" data-cg="${i}" data-cgf="loc" data-ph="${t("集合地點")}">${esc(g.loc)}</span></span>
      <span class="cs-spacer"></span>
      <span class="cs-gt cut-edit" contenteditable draggable="false" data-cg="${i}" data-cgf="time" data-ph="07:00">${esc(g.time)}</span>
      <button class="cs-gdel" data-cgdel="${i}" aria-label="${t("刪除組別")}">✕</button>
    </div>`;
  });
